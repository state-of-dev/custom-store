  ...
  return (
    <div className="flex flex-col justify-between h-full overflow-hidden">
      <CartContainer className="flex justify-between px-2 text-sm text-muted-foreground">
        <span>Productos</span>
        <span>{cart.lines.length} artículos</span>
      </CartContainer>
...
export default function CartModal() {
          ...
          >
            <div className="flex flex-row gap-6">
              ...
              <div className="flex flex-col flex-1 gap-2 justify-center 2xl:gap-3">
                <span className="text-lg font-semibold 2xl:text-xl">El carrito está vacío</span>
                <p className="text-sm text-muted-foreground hover:underline">Comienza a comprar para empezar</p>
              </div>
            </div>
          </Link>
        </CartContainer>
  ...
  return (
    <>
      <Button aria-label="Abrir carrito" onClick={openCart} className="uppercase" size={"sm"}>
        <span className="max-md:hidden">carrito</span> ({cart?.totalQuantity || 0})
      </Button>
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            >
              <div className="flex flex-col py-3 w-full rounded bg-muted md:py-4">
                <CartContainer className="flex justify-between items-baseline mb-10">
                  <p className="text-2xl font-semibold">Carrito</p>
                  <Button size="sm" variant="ghost" aria-label="Cerrar carrito" onClick={closeCart}>
                    Cerrar
                  </Button>
                </CartContainer>
